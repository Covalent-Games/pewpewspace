...
public class ShipMovement : MonoBehaviour {
	...
	void RotateTurret(){
        ...
        if (ship.Target != null) {
            UpdateTargetingLine(turret, Vector3.Distance(transform.position, ship.Target.position));
        } else {
            // In this case the player has no target and is aiming to empty space, so give them a moderate line to aim with.
            UpdateTargetingLine(turret, 25f);
        }

        if (turret != null) {
            if (thisRotation.magnitude < 0.85f) {
                if (ship.Target != null) {
    ...
    private void UpdateTargetingLine(Transform turret, float length) {
